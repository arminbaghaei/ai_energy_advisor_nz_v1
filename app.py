...
.certificate-kicker {font-size: 0.86rem; color:#0F766E; font-weight:800; letter-spacing:0.12em; text-transform:uppercase;}
.certificate-title {font-size: clamp(2.05rem, 4.4vw, 2.65rem); font-weight: 900; color: #0F172A; margin: 0.4rem 0 0.2rem 0; letter-spacing: -0.02em;}
.certificate-subtitle {font-size: 1.10rem; color: #475569; margin-bottom: 0.45rem;}
.certificate-company {font-size: 1.02rem; color:#0F766E; font-weight:800; margin-bottom:1.1rem;}
.certificate-name {font-size: clamp(1.85rem, 4.4vw, 2.4rem); font-weight: 900; color: #0F766E; margin: 0.75rem auto; padding-bottom:0.42rem; border-bottom: 2px solid #99F6E4; max-width: 560px;}
.certificate-small {font-size: 1.02rem; color: #334155; line-height: 1.55; max-width:620px; margin-left:auto; margin-right:auto;}
.certificate-meta {display:flex; justify-content:center; gap:0.65rem; flex-wrap:wrap; margin-top:1rem;}
.certificate-pill {padding:0.5rem 0.82rem; border-radius:999px; background:#ECFDF5; border:1px solid #A7F3D0; color:#065F46; font-weight:700; font-size:0.90rem;}
.certificate-footer {margin-top:1.15rem; font-size:0.90rem; color:#475569;}
.certificate-disclaimer {font-size:0.82rem; color:#64748B; margin-top:0.75rem;}
...
.certificate-medal::before, .certificate-medal::after {
    content:"";
    position:absolute;
    top:62px;
    width:34px;
...
.certificate-medal::before {left:20px; transform:rotate(14deg); clip-path:polygon(0 0,100% 0,75% 100%,50% 76%,25% 100%);}
.certificate-medal::after {right:20px; transform:rotate(-14deg); clip-path:polygon(0 0,100% 0,75% 100%,50% 76%,25% 100%);}
.certificate-medal-face {
    width:78px;
    height:78px;
    border-radius:50%;
    background: radial-gradient(circle at 35% 30%, #FFF7CC 0%, #FACC15 48%, #B45309 100%);
...
.header-grid-single { grid-template-columns: minmax(0, 1fr) !important; }
.sidebar-brand { margin-top: 0 !important; padding-top: 0.2rem !important; padding-bottom: 0.85rem; border-top: none !important; border-bottom: 1px solid #E2E8F0; margin-bottom: 1rem; }
</style>
"""
...
def _sheet_has_expected_columns(existing_data: pd.DataFrame | None) -> bool:
    """Return True only when the Google Sheet is already in row-based response format.

    Some sheets are initially set up with variable names listed down column A. That looks
    readable to a person, but it is not the tabular format required for appending
    participant responses. This check lets the app safely initialise the sheet into
    the correct horizontal-header format on the first successful save.
    """
    if existing_data is None or existing_data.empty:
        return False
    required = {"participant_id", "submission_time", "score"}
    return required.issubset(set(map(str, existing_data.columns)))


def get_next_participant_id(conn) -> str:
    """Generate a simple anonymous participant ID based on saved response rows."""
    try:
        existing_data = conn.read(ttl=0)
        if not _sheet_has_expected_columns(existing_data):
            return "Participant 1"
        valid_rows = existing_data.dropna(how="all")
        return f"Participant {len(valid_rows) + 1}"
    except Exception:
        return "Participant 1"
...
def save_response_to_gsheet(submission: dict) -> None:
    ...
    The target sheet must be a normal table: field names across the first row, one
    participant per row. If the sheet is blank or has the old vertical-header layout,
    this function initialises it into the correct format automatically.
    """
    conn = st.connection("gsheets", type=GSheetsConnection)
    existing_data = conn.read(ttl=0)
    ordered_columns = list(submission.keys())
    new_row = pd.DataFrame([submission], columns=ordered_columns)

    if _sheet_has_expected_columns(existing_data):
        existing_data = existing_data.dropna(how="all")
        # Keep any existing compatible columns, then add any new app columns in order.
        all_columns = list(dict.fromkeys(list(existing_data.columns) + ordered_columns))
        updated_data = pd.concat([existing_data.reindex(columns=all_columns), new_row.reindex(columns=all_columns)], ignore_index=True)
    else:
        # Initialise or repair the sheet into a proper response table.
...
def save_anonymous_progress_if_needed(savings: dict | None = None, ranked: list | None = None, result_text: str | None = None) -> bool:
    ...
    try:
        ...
        st.session_state["save_timestamp"] = submission.get("submission_time", datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
...
def logo_header() -> None:
    st.markdown(
        f"""
        <div class="header-grid header-grid-single">
            <div class="hero">
                <h1>{APP_TITLE}</h1>
                <p>{TAGLINE}</p>
...
def plan_screen() -> None:
    ...
    st.markdown(
        ...
        <div class='certificate-card'>
            <div class='certificate-medal-wrap'><div class='certificate-medal'><div class='certificate-medal-face'>✓</div></div></div>
            <div class='certificate-kicker'>Completion recognition</div>
...
def render_save_status() -> None:
    if st.session_state.get("response_saved"):
        st.markdown(
            f"<div class='save-status-ok'><strong>Responses saved to Google Sheets.</strong><br>Participant ID: {st.session_state.get('participant_id', 'not available')}<br>Saved at: {st.session_state.get('save_timestamp', 'not available')}</div>",
            unsafe_allow_html=True,
        )
    elif st.session_state.get("save_error"):
        st.markdown(
            f"<div class='save-status-error'><strong>Google Sheets save failed.</strong><br>{st.session_state.get('save_error')}<br><br>The recommendations will still open, but this response has not been recorded. Check Streamlit Secrets and confirm the Google Sheet is a normal table with headers across row 1.</div>",
            unsafe_allow_html=True,
        )
    else:
        return
...
def render_sidebar_context() -> None:
    """Show page-specific guidance in the sidebar as users progress through the tool."""
    stage = st.session_state.get("stage", "welcome")
    context = {
        "welcome": {
            "label": "Start page",
            "title": "Home energy challenge",
            "text": "Start the tool, estimate your bill leak, then move through the inspection and roadmap."
        },
        "money": {
            "label": "Step 1 of 4",
            "title": "Money leak scan",
            "text": "Enter the monthly bill, household size, home type, and main bill pressure to reveal the first saving opportunity."
        },
        "profile": {
            "label": "Step 2 of 4",
            "title": "Household setup",
            "text": "Set the assumptions that personalise calculations: tenure, system type, shower time, lights, standby devices, and tariff."
        },
        "inspection": {
            "label": "Step 3 of 4",
            "title": "Seven-leak inspection",
            "text": "Open each area, answer its questions, and watch completed areas turn green before moving to the next section."
        },
        "plan": {
            "label": "Step 4 of 4",
            "title": "Action roadmap",
            "text": "Review the ranked recommendations, choose a 30-day challenge, and download the roadmap if needed."
        },
    }.get(stage, {})
    if not context:
        return
    st.sidebar.markdown(
        f"""
        <div class='gamified-status-card'>
            <div class='counter-label'>{context['label']}</div>
            <h4>{context['title']}</h4>
            <p>{context['text']}</p>
        </div>
        """,
...
def sidebar_status() -> None:
    st.sidebar.markdown(_sidebar_logo_html(), unsafe_allow_html=True)
    st.sidebar.title("Home-energy guide")
...
def inspection_screen() -> None:
    ...
    if len(st.session_state.completed_hotspots) >= 7:
        st.info("You have completed all seven checks. Click the button below to submit the anonymous responses to Google Sheets and open the recommendations.")
        render_save_status()
        if st.button("Submit responses and view my recommendations", type="primary", use_container_width=True):
            inputs = current_household_inputs()
            savings = estimate_all_savings(inputs)
            ranked = current_ranked_actions()
            result_text = score_label(st.session_state.get("score", 0))
